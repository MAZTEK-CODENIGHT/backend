...
router.post("/estimate", (req, res, next) => CheckoutController.estimatePrice(req, res, next));

// Kullanıcının sipariş geçmişi
// GET /api/checkout/history/:userId?limit=10&status=completed
router.get("/history/:userId", (req, res, next) => CheckoutController.getOrderHistory(req, res, next));
...
router.get("/:orderId", (req, res, next) => CheckoutController.getOrderStatus(req, res, next));

// Sipariş iptali
// DELETE /api/checkout/:orderId
router.delete("/:orderId", (req, res, next) => CheckoutController.cancelOrder(req, res, next));
